#let exp-7d = (
  entry-header-args: (
    top-left: [*Seven D Construction*],
    top-right: [Jan 2026 - Aug 2026],
    bottom-left: [Software Engineer · Project-based Contract (Remote)],
    bottom-right: [Chicago, Illinois, USA / Mexico],
  ),
)

#let exp-coppel = (
  entry-header-args: (
    top-left: [*Coppel (Genius Arena Hackathon — Talent Land 2025)*],
    top-right: [Apr 2025],
    bottom-left: [4th Place & Team Lead],
    bottom-right: [Guadalajara, Jalisco, Mexico],
  ),
)

#let exp-azteca = (
  entry-header-args: (
    top-left: [*Banco Azteca (Genius Arena Hackathon — Talent Land 2026)*],
    top-right: [Apr 2026],
    bottom-left: [Participant & Team Lead],
    bottom-right: [Santa Fe, CDMX, Mexico],
  ),
)

#let exp-pepsico = (
  entry-header-args: (
    top-left: [*PepsiCo*],
    top-right: [May 2024 - Aug 2024],
    bottom-left: [Software Developer · Project-based Contract (Remote)],
    bottom-right: [Azcapotzalco, CDMX, Mexico],
  ),
)

#let exp-flacks = (
  entry-header-args: (
    top-left: [*Flack's Barber Shop*],
    top-right: [Oct 2022 - Apr 2024],
    bottom-left: [Software Developer · Project-based Contract],
    bottom-right: [Córdoba, Veracruz, Mexico],
  ),
)
