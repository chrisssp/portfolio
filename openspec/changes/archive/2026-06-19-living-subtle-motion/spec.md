...
- **Keyframe**:
  ...
  ```
- **Reduced motion**: `animation: none`
- **Edge cases**: Only the active/current timeline dot pulses. Past dots are static.
...
- **Keyframe**: Reuses `.animate-float` with different duration classes
- **Reduced motion**: `animation: none`
- **Edge cases**: Different durations prevent in-phase alignment — photos always move independently.
...
- **Component**: Any tech badge container
- **Trigger**: Scroll-reveal entry (when badge enters viewport)
...
- **Edge cases**: One-shot animation, not looping. Triggers once per badge via IntersectionObserver (reuses existing pattern). Badges that enter simultaneously get the same timing — no stagger needed.
